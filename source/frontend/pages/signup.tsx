...
import {Colors, StartLogo, StyledContainer, InnerContainer, PageTitle, Spacer, StyledFormArea, StyledTextInput, StyledInputLabel, LeftIcon, RightIcon, MsgBox, Line} from "../components/style";
...
export class SignUp extends Component<SignUpProps, SignUpState> {
  ...
  validate = () => {
    {/** Add more cases if needed 
      ...
      */}
    if (this.state.user_id === "" || this.state.password === "" || this.state.confirmPassword === "" || this.state.email === "") {
    ...
    if (!emailRegex.test(this.state.email)) {
      this.setState({message: "Please enter a valid email address (example@example.com)", messageType:"ERROR"});
      return false;
    }

    if (this.state.password.length < 8) {
      this.setState({message: "Password must be at least 8 characters long", messageType:"ERROR"});
      return false;
    }
